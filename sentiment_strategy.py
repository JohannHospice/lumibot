...
class SentimentStrategy(Strategy):
    ...
    def initialize(self, symbol: str, cash_at_risk: float, sleeptime: str, api: REST, days_prior: int = 3):
        self.symbol = symbol
        ...
        self.get_sentiment_and_news_cached = GetSentimentAndNewsCached(symbol, api.get_news)
    ...
    def get_sentiment(self) -> Tuple[float, str]:
        ...
        _, probability, sentiment, number_of_news = self.get_sentiment_and_news_cached.get_news_and_sentiment(self.get_dates())
        self.number_of_news = self.number_of_news + number_of_news
        self.number_of_news_calls = self.number_of_news_calls + 1
        return probability, sentiment
    
    def get_average_number_of_news(self) -> float:
    ...
    def on_strategy_end(self):
        print(f"\n\nMoyenne d'articles par jour: {self.get_average_number_of_news()}")
